...
{
    g_graphics->colour(0, 128, 255);
    g_graphics->print_text(fixed_font, "   ", column * manual_column_width, -1);
    switch (category) {
    case KeywordCategory::OPERATORS:
        g_graphics->print_text(manual_font, "Operators\r", -1, -1);
        break;
    case KeywordCategory::KEYBOARD_AND_MOUSE:
        g_graphics->print_text(manual_font, "Keyboard & Mouse\r", -1, -1);
        break;
    case KeywordCategory::MATHS_AND_NUMBERS:
        g_graphics->print_text(manual_font, "Maths & Numbers\r", -1, -1);
        break;
    case KeywordCategory::FILE_IO:
        g_graphics->print_text(manual_font, "File I/O\r", -1, -1);
        break;
    case KeywordCategory::GRAPHICS_2D:
        g_graphics->print_text(manual_font, "2D g_graphics\r", -1, -1);
        break;
    case KeywordCategory::GRAPHICS_3D:
        g_graphics->print_text(manual_font, "3D g_graphics\r", -1, -1);
        break;
    case KeywordCategory::CONDITIONAL_LOOPING:
        g_graphics->print_text(manual_font, "Conditional & Looping\r", -1, -1);
        break;
    case KeywordCategory::FN_AND_PROC:
        g_graphics->print_text(manual_font, "Functions and Procedures\r", -1, -1);
        break;
    case KeywordCategory::VARIABLES_TYPES:
        g_graphics->print_text(manual_font, "Variables, Structured Types and Data\r", -1, -1);
        break;
    case KeywordCategory::STRING:
        g_graphics->print_text(manual_font, "Strings & Text Output\r", -1, -1);
        break;
    case KeywordCategory::DEBUGGING:
        g_graphics->print_text(manual_font, "Debugging & Execution\r", -1, -1);
        break;
    case KeywordCategory::BOOLEAN:
        g_graphics->print_text(manual_font, "Boolean Logic\r", -1, -1);
        break;
    }
    ...
    for (auto it = keywords.begin(); it != keywords.end(); ++it) {
        if ((*it).category == category) {
            ...
            if ((*it).description.length() == 0) {
                ...
                g_graphics->print_text(fixed_font, "*", -1, -1);
            } else {
                g_graphics->print_text(fixed_font, " ", -1, -1);
            }
            g_graphics->colour(255, 255, 255);
            auto kw = (*it).keywordLong;
            g_graphics->print_text(fixed_font, kw, -1, -1);
            int a = 15 - static_cast<int>(kw.length());
            for (int i = 0; i < a; i++) {
                kw = " " + kw;
            }
            g_graphics->print_text(fixed_font, "\r", -1, -1);

            // Are we out of space?
            if (g_graphics->get_cursor_y() > g_graphics->get_actual_height() - 100) {
                column++;
                g_graphics->print_text(fixed_font, "", column * manual_column_width, 50);
            }
...
void Debugger::debugger_manual()
{
    ...
    while (true) {
        ...
        if (updated) {
            ...
            g_graphics->colour(255, 255, 255);
            g_graphics->print_text(fixed_font, "Use Up/Down/PgUp/PgDown & Enter  ", 20, g_graphics->get_actual_height() - 30);
            g_graphics->colour(255, 0, 0);
            g_graphics->print_text(fixed_font, "* ", -1, -1);
            g_graphics->colour(255, 255, 0);
            g_graphics->print_text(manual_font, "No help available yet", -1, -1);

            g_graphics->print_text(manual_font, "", -1, 40);
            int index = 0;
            column = 0;
            g_graphics->print_text(fixed_font, "", 0, 50);
        ...
        g_graphics->poll();
        if (g_graphics->inkey(-17)) {
            exit(0);
        } else if (g_graphics->inkey(-58)) {
            while (g_graphics->inkey(-58))
                g_graphics->poll();
            selected--;
            updated = true;
        } else if (g_graphics->inkey(-42)) {
            while (g_graphics->inkey(-42))
                g_graphics->poll();
            selected++;
            updated = true;
        } else if (g_graphics->inkey(-64)) {
            while (g_graphics->inkey(-64))
                g_graphics->poll();
        ...
        } else if (g_graphics->inkey(-79)) {
            while (g_graphics->inkey(-79))
                g_graphics->poll();
        ...
        } else if (g_graphics->inkey(-74)) {
            // RETURN
            debugger_manual_keyword(selected_keyword);
            updated = true;
        } else if (g_graphics->inkey(-114) || g_graphics->inkey(-115) || g_graphics->inkey(-31)) {
            g_graphics->restore();
            return;
        }
        if (selected < 0)
...
void Debugger::debugger_manual_keyword(std::string keyword)
{
    ...
    debugger_options(8, 2);
    g_graphics->print_text(manual_font, "\r\r", -1, -1);
    g_graphics->set_margin(20);

    // Keyword
    g_graphics->colour(255, 255, 255);
    g_graphics->print_text(fixed_font + 2, kw->keywordLong + " ", -1, -1);

    // Category
    g_graphics->colour(255, 80, 0);
    switch (kw->category) {
    case KeywordCategory::KEYBOARD_AND_MOUSE:
        g_graphics->print_text(manual_font + 2, "(Keyboard & Mouse)\r\r", -1, -1);
        break;
    case KeywordCategory::MATHS_AND_NUMBERS:
        g_graphics->print_text(manual_font + 2, "(Maths & Numbers)\r\r", -1, -1);
        break;
    case KeywordCategory::GRAPHICS_2D:
        g_graphics->print_text(manual_font + 2, "(2D g_graphics)\r\r", -1, -1);
        break;
    case KeywordCategory::GRAPHICS_3D:
        g_graphics->print_text(manual_font + 2, "(3D g_graphics)\r\r", -1, -1);
        break;
    case KeywordCategory::OPERATORS:
        g_graphics->print_text(manual_font + 2, "(Operators)\r\r", -1, -1);
        break;
    case KeywordCategory::FILE_IO:
        g_graphics->print_text(manual_font + 2, "(File I/O)\r\r", -1, -1);
        break;
    case KeywordCategory::CONDITIONAL_LOOPING:
        g_graphics->print_text(manual_font + 2, "(Conditional & Looping)\r\r", -1, -1);
        break;
    case KeywordCategory::FN_AND_PROC:
        g_graphics->print_text(manual_font + 2, "(Functions and Procedures)\r\r", -1, -1);
        break;
    case KeywordCategory::VARIABLES_TYPES:
        g_graphics->print_text(manual_font + 2, "(Variables, Structured Types and Data)\r\r", -1, -1);
        break;
    case KeywordCategory::STRING:
        g_graphics->print_text(manual_font + 2, "(Strings & Text Output)\r\r", -1, -1);
        break;
    case KeywordCategory::DEBUGGING:
        g_graphics->print_text(manual_font + 2, "(Debugging & Execution)\r\r", -1, -1);
        break;
    case KeywordCategory::BOOLEAN:
        g_graphics->print_text(manual_font + 2, "(Boolean Logic)\r\r", -1, -1);
        break;
    }
    ...
    if (kw->description.length() > 0) {
        ...
        g_graphics->print_text(manual_font, "Description:\r\r", -1, -1);
        g_graphics->colour(255, 255, 255);
        g_graphics->print_text(manual_font, kw->description, -1, -1);
        g_graphics->print_text(manual_font, "\r\r", -1, -1);
    }

    // Syntax
    g_graphics->colour(128, 128, 128);
    g_graphics->print_text(manual_font, "Syntax:\r\r", -1, -1);
    g_graphics->colour(255, 255, 255);
    g_graphics->print_text(fixed_font, kw->keyword, -1, -1);
    ...
    if (kw->arguments.length() > 0) {
        g_graphics->colour(128, 128, 128);
        g_graphics->print_text(manual_font, "Arguments:\r\r", -1, -1);
        g_graphics->colour(255, 255, 255);
        g_graphics->print_text(manual_font, kw->arguments, -1, -1);
        g_graphics->print_text(manual_font, "\r\r", -1, -1);
    }
    ...
    if (kw->result.length() > 0) {
        g_graphics->colour(128, 128, 128);
        g_graphics->print_text(manual_font, "Result:\r\r", -1, -1);
        g_graphics->colour(255, 255, 255);
        g_graphics->print_text(manual_font, kw->result, -1, -1);
        g_graphics->print_text(manual_font, "\r\r", -1, -1);
    }
    ...
    if (kw->examples.length() > 0) {
        g_graphics->colour(128, 128, 128);
        g_graphics->print_text(manual_font, "Examples:\r\r", -1, -1);
        g_graphics->colour(255, 255, 255);
        g_graphics->print_text(fixed_font, kw->examples, -1, -1);
    }

    g_graphics->colour(255, 255, 0);
    g_graphics->print_text(10, "\r\r(Hit Space to return)", -1, -1);
    ...
    while (true) {
        ...
        if (g_graphics->inkey(-17)) {
            exit(0);
        } else if (g_graphics->inkey(-99) || g_graphics->inkey(-114) || g_graphics->inkey(-115) || g_graphics->inkey(-31)) {
            return;
        }
    }
}
