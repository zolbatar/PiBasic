...
void Compiler::compile_node_token(struct AST* ast, bool expression)
{
    ...
    case ARRAYSIZE: {
        ...
        vm->insert_instruction(line_number, file_number, write, Bytecodes::ARRAYSIZE, var_id);
    ...
    case SWAP_I: {
        ...
        vm->insert_instruction(line_number, file_number, write, Bytecodes::SWAP_I, var_id1);
        break;
    }
    case SWAP_F: {
        var_name = ast->items[0]->string;
        ...
        auto var_id2 = find_or_create_variable(VariableScope::NOSCOPE, false);
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I_VAR, var_id2);
        vm->insert_instruction(line_number, file_number, write, Bytecodes::SWAP_F, var_id1);
        break;
    }
    case SWAP_S: {
        var_name = ast->items[0]->string;
        ...
        auto var_id2 = find_or_create_variable(VariableScope::NOSCOPE, false);
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I_VAR, var_id2);
        vm->insert_instruction(line_number, file_number, write, Bytecodes::SWAP_S, var_id1);
        break;
    }
    ...
    case OSCLI:
        ...
        break;
    case TRACEON:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::TRACEON);
        break;
    case TRACEOFF:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::TRACEOFF);
        break;
    case BREAKPOINT:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::BREAKPOINT);
        break;
    case END:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::HALT);
        break;
    case TIME:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::TIME);
        stack_push(Type::INTEGER);
        break;
    case TIMES:
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::TIMES);
        stack_push(Type::STRING);
        break;
    case BOOLFALSE:
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, 0);
        stack_push(Type::INTEGER);
        break;
    case BOOLTRUE:
...
void Compiler::compile_node_token_chain(struct AST* ast)
{
    if (ast->items.size() == 1) {
        ...
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, count);
    }
    compile_node(ast->items[0], true);
    stack_pop();
    vm->insert_bytecode(line_number, file_number, write, Bytecodes::CHAIN);
}
