...
void Compiler::compile_node_token_looping(struct AST* ast)
{
    ...
    case GOTO:
        if (!write) {
            g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::GOTO, 0);
        } else {
            auto pc = (*line_no_to_bytecode.find(ast->items[0]->integer)).second;
            g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::GOTO, pc);
        }
        assert(stack_size() == 0);
        break;
    case GOSUB:
        if (!write) {
            g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::GOSUB, 0);
        } else {
            auto pc = (*line_no_to_bytecode.find(ast->items[0]->integer)).second;
            g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::GOSUB, pc);
        }
        assert(stack_size() == 0);
        break;
    default:
...
{
    UINT32 start_pc = g_vm->helper_bytecodes().pc;
    IfStatement s;
    s.end_pc = 0;
    s.false_pc = 0;
    if (write) {
    ...
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::JNE, s.false_pc);

    // This is the truth section
    compile_node(ast->items[1], false);
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::JUMP, s.end_pc);

    // and false section (the else bit)
    if (!write) {
        s.false_pc = g_vm->helper_bytecodes().pc;
    }
    if (ast->items.size() == 3) {
        compile_node(ast->items[2], false);
    }
    if (!write) {
        s.end_pc = g_vm->helper_bytecodes().pc;
    }
...
void Compiler::compile_node_forin(struct AST* ast)
{
    ...
    // PC
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, g_vm->helper_bytecodes().pc + 2);

    // Actual FOR loop token
    switch (saved_type) {
    case Type::INTEGER_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::FORIN_I, var_id);
        break;
    case Type::REAL_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::FORIN_F, var_id);
        break;
    case Type::STRING_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::FORIN_S, var_id);
        break;
    }
    ...
    switch (saved_type) {
    case Type::INTEGER_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::NEXTIN_I, var_id);
        break;
    case Type::REAL_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::NEXTIN_F, var_id);
        break;
    case Type::STRING_ARRAY:
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::NEXTIN_S, var_id);
        break;
    }
}
...
void Compiler::compile_node_for(struct AST* ast)
{
    ...
    if (var_type == Type::INTEGER) {
        ...
        g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::SUBTRACT_I);
        stack_pop();
        ...
        // Actual FOR loop token
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::FOR_I, var_id);

        // Process loop content
        compile_node(body, false);

        // And next
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::NEXT_I, var_id);
    } else {
        ...
        g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::SUBTRACT_F);
        stack_pop();

        // Step
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::LOAD_F, constant_float_create(1.0));

        // PC
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, g_vm->helper_bytecodes().pc + 2);

        // Actual FOR loop token
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::FOR_F, var_id);

        // Process loop content
        compile_node(body, false);

        // And next
        g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::NEXT_F, var_id);
    }
}
...
{
    g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::REPEAT);
    ...
    g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::JNEREP);
    stack_pop();
}
...
void Compiler::compile_node_while(struct AST* ast)
{
    ...
    // We don't know the amount to jump ahead yet
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::JNE, s.end_pc);
    stack_pop();

    // This is the truth section
    compile_node(body, false);
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::JUMP, start_pc);
...
void Compiler::compile_node_case(struct AST* ast)
{
    ...
    // This is the condition
    g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::CASE_C);
...
void Compiler::compile_node_otherwise(struct AST* action)
{
    ...
    // We don't know the amount to jump ahead yet
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::CJUMPT, s.end_pc);
...
void Compiler::compile_node_when_single(struct AST* var, struct AST* expression, struct AST* action)
{
    ...
    // We don't know the amount to jump ahead yet
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, Bytecodes::JNE, s.end_pc);
    g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, Bytecodes::CASE_S);
