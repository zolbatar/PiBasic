...
void Compiler::compile_node_variable1d_dim(struct AST* ast, int var_id)
{
    if (var_type == Type::TYPE_ARRAY) {
        ...
        g_vm->insert_bytecode(line_number, file_number, write, Bytecodes::MULTIPLY_I);
        ...
        stack_pop();
        g_vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, 1); // 1 dimension
        insert_instruction_based_on_type(
            {
...
void Compiler::compile_node_variable1d_expression(struct AST* ast)
{
    ...
    case Type::TYPE_ARRAY: {
        ...
        g_vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, static_cast<int>(c.field_id)); // Field index
        g_vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, static_cast<int>(c.field_count)); // Field count
        insert_instruction_based_on_type(
            {
