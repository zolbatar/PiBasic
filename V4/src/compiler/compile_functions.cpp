...
void Compiler::compile_node_token_functions(struct AST* ast, bool expression)
{
    ...
    case RETURN: {
        ...
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::RETURN);
        break;
    }
...
void Compiler::compile_node_function_call(struct AST* ast, bool expression)
{
    ...
    if (!write) {
        ...
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CALL, 0);
    } else {
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CONST_I, (*g).second.id);
        vm->insert_instruction(line_number, file_number, write, Bytecodes::CALL, (*g).second.pc_start);
    }
    ...
    if (!expression && f.type != Type::NOTYPE) {
        ...
        vm->insert_bytecode(line_number, file_number, write, Bytecodes::NOP);
...
void Compiler::compile_node_function(struct AST* ast)
{
    ...
    if (!write) {
        vm->insert_instruction(line_number, file_number, write, Bytecodes::JUMP, 0);
    } else {
        vm->insert_instruction(line_number, file_number, write, Bytecodes::JUMP, (*g).second.pc_end);
    }
