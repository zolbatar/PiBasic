...
{
    auto t = m.find(type);
    g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, (*t).second);
}
...
void Compiler::insert_instruction_based_on_type(std::map<Type, Bytecodes> m, Type type, UINT32 value)
{
    ...
    g_vm->helper_bytecodes().insert_instruction(line_number, file_number, write, (*t).second, value);
}
...
void Compiler::comparison(Bytecodes i, Bytecodes f, Bytecodes s)
{
    switch (peek_type()) {
    case Type::INTEGER:
        g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, i);
        break;
    case Type::REAL:
        g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, f);
        break;
    case Type::STRING:
        g_vm->helper_bytecodes().insert_bytecode(line_number, file_number, write, s);
        break;
    default:
