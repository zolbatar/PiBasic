...
class MyParserErrorListener : public antlr4::BaseErrorListener {
        ...
        std::exception_ptr e) override
    {
        ...
        if (error_count == 5) {
            ...
            g_env.graphics.colour(0, 255, 0);
            g_env.graphics.print_console(std::to_string(fl.line));
            g_env.graphics.colour(128, 128, 128);
            g_env.graphics.print_console(":");
            g_env.graphics.colour(0, 255, 0);
...
void MyParser::parse_and_compile(Compiler* compiler, bool interactive)
{
    // Setup
    parse_errors = false;
    error_count = 0;
    ...
    if (!stream.is_open()) {
        throw std::runtime_error("File '" + filename + "'not found\r");
    }
    ss << stream.rdbuf();
    stream.close();

    // Any INSTALLs? These are only processed in the FIRST file, and ignored in subsequent ones.
    std::string line;
    int i = 0;
