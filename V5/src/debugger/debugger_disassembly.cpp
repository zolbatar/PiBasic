	...
	while (true) {
		if (rerender) {
			...
			if (pos >= 0 && pos < g_vm->helper_bytecodes().get_size()) {
				...
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, "File: ", -1, -1);
				g_env.graphics.colour(255, 255, 255);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, flp.filename, -1, -1);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, "\r", -1, -1);
				g_env.graphics.colour(128, 128, 128);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, "Line (", -1, -1);
				g_env.graphics.colour(255, 255, 255);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, std::to_string(flp.line), -1, -1);
				g_env.graphics.colour(128, 128, 128);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, "): ", -1, -1);
				g_env.graphics.colour(255, 255, 255);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, concat_file_cache[cur_bc.line_number - 1], -1, -1);
				g_env.graphics.print_text(disassembly_font, disassembly_font_size, "\r\r", -1, -1);
				...
				for (UINT32 i = 0; i < debugger_lines; i++) {
					if (pos + i < g_vm->helper_bytecodes().get_size()) {
						...
						g_env.graphics.colour(255, 255, 255);
						g_env.graphics.print_text(disassembly_font, disassembly_font_size, s.header, -1, -1);
						g_env.graphics.colour(255, 255, 0);
						g_env.graphics.print_text(disassembly_font, disassembly_font_size, s.opcode, -1, -1);
						...
						g_env.graphics.print_text(disassembly_font, disassembly_font_size, s.type, -1, -1);
						...
						if (s.operand.length() > 0) {
							...
							g_env.graphics.print_text(disassembly_font, disassembly_font_size, s.operand, -1, -1);
						}

						g_env.graphics.print_text(prop_font, prop_font_size, "\r", -1, -1);
					}
				}
			}
			rerender = false;
