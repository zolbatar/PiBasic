...
public:
    Vector3() {};
    Vector3(VM_FLOAT x, VM_FLOAT y, VM_FLOAT z)
        : x(x)
        , y(y)
        , z(z) {};
    VM_FLOAT x;
...
public:
    ...
    void process(RenderType render_type, WorldState& state, std::vector<RenderTriangle>& render_list);
    ...
    void make_colour(Colour* original, Colour* result, VM_FLOAT directional_ratio, VM_FLOAT distance, WorldState& state);
    void create_normals();
...
public:
    Object() {};
    void render(std::vector<Shape>& shapes, WorldState& state, std::vector<RenderTriangle>& render_list);
    VM_INT shape_index;
    Vector3 position;
    Vector3 rotation;
    VM_FLOAT scale;
...
public:
    ...
    {
        render_list.reserve(MaximumTriangles);
