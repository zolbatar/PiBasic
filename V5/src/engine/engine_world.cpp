...
void World::render()
{
...
#ifndef RISCOS
	// With hardware acceleration we really want to create a texture and then blit it for performance
	auto tex = SDL_CreateTexture(g_env.graphics.get_renderer(),
		SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC | SDL_TEXTUREACCESS_TARGET,
		g_env.graphics.get_actual_width(), g_env.graphics.get_actual_height());
	SDL_SetRenderTarget(g_env.graphics.get_renderer(), tex);
	...
	SDL_RenderCopy(g_env.graphics.get_renderer(), tex, NULL, NULL);
#endif
}
