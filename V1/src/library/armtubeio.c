...
int _stat(char *filename, struct stat *st)
{
    ...
    errptr = _swix(OS_Find, _INR(0, 1) | _OUT(0) | _OUT(_FLAGS), 0x40, filename, &handle, &flags);
    ...
    if (handle != 0)
    {
        ...
        st->st_size = _fgetextent(handle);

        errptr = _swix(OS_Find, _INR(0, 1), 0, handle);
...
// hack as feof() doesn't seem to work properly
int myfeof(FILE *fp)
{
    long fptr, end;
